...
def read_event_data(DATADIR,game_id):
    ...
    events = pd.read_csv(eventfile) # read data
    return events
...
def tracking_data(DATADIR, game_id, teamname):
    ...
    '''
    trace_file =  os.path.join(DATADIR, f'Sample_Game_{game_id}', f'Sample_Game_{game_id}_RawTrackingData_{teamname}_Team.csv')
    # First:  deal with file headers so that we can get the player names correct
    csvfile =  open(trace_file, 'r') # create a csv file reader
    ...
    tracking = pd.read_csv(trace_file, names=columns, index_col='Frame', skiprows=3)
    return tracking
