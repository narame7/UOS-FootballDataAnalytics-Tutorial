...
### `Week 2` — 데이터 불러오기
- `1-load-statistic-data.ipynb` — 통계 데이터를 일곱 곳에서 불러와 살펴보기 (2026/27 시즌 기준)
  - ClubElo: 웹페이지 표를 `pandas.read_html`로 읽어 리그별 Elo 순위와 팀별 최근 경기 Elo 변화 보기, 팀 페이지의 차트 데이터로 최근 4년 Elo 추이 그래프 그리기 (API는 2026년 9월 현재 응답하지 않음)
  - 프리미어리그 공식 사이트 API(비공식): 순위표, 지표별 선수 순위, 경기 결과와 관중 수
  - UEFA 챔피언스리그 공식 사이트 API(비공식): 리그 페이즈 순위표, 경기 결과와 관중 수, 선수·팀 지표별 순위(득점, 슈팅, 패스, 뛴 거리, 최고 속도)
  - 분데스리가 공식 사이트(비공식, Firebase): 순위표, AWS Match Facts 피지컬 지표(최고 속도, 스프린트, 뛴 거리, 인텐시브 런)와 공격·수비 지표별 선수 순위, 구단 순위, 선수 개인 시즌 기록
  ...
  - FBref, Transfermarkt, Sofascore, ESPN, FotMob, WhoScored는 봇 차단 때문에 Colab에서 안 되어 다루지 않습니다.
- `2-load-event-and-tracking-data.ipynb` — Metrica Sports 샘플 이벤트·트래킹 데이터(CSV)를 불러와 좌표 변환, 속도 계산, 장면 시각화, 영상 저장
- 필요 라이브러리: `pandas`, `requests`, `lxml`, `matplotlib`, `seaborn`, `scipy`, `soccerdata` (영상 저장에는 `ffmpeg` 필요)
